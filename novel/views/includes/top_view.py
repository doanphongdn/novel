...
class TopViewTemplateInclude(BaseTemplateInclude):
    ...
    def prepare_include_data(self):
        ...
        novel_daily = CacheManager(Novel, limit=7, order_by=["view_daily"]).get_from_cache(get_all=True)
        novel_monthly = CacheManager(Novel, limit=7, order_by=["view_monthly"]).get_from_cache(get_all=True)
        novel_all = CacheManager(Novel, limit=7, order_by=["view_total"]).get_from_cache(get_all=True)
